...
community_data <- community_data |>
  mutate(region = sapply(tract_fips10, get_state))|>
...
plot_usmap(
   data = tobacco_sum_by_state, values = "TotalTobaccoStores", lines = "blue"
 ) + 
...
plot_usmap(
   data = perform_art_sum_by_state, values = "total_perform_art", lines = "blue"
 ) + 
   scale_fill_continuous(
...
plot_usmap(
   data = lib_sum_by_state, values = "total_lib", lines = "blue"
 ) + 
   scale_fill_continuous(
     low = "white", high = "purple", name = "total per 1000", label = scales::comma
   ) + 
...
plot_usmap(
   data = amuse_sum_by_state, values = "total_amuse", lines = "blue"
 ) + 
   scale_fill_continuous(
     low = "white", high = "purple", name = "total per 1000", label = scales::comma
   ) + 
...
plot_usmap(
   data = rec_sum_by_state, values = "total_rec", lines = "blue"
 ) + 
   scale_fill_continuous(
     low = "white", high = "purple", name = "total per 1000", label = scales::comma
   ) + 
...
plot_usmap(
   data = fit_sum_by_state, values = "total_fit", lines = "blue"
 ) + 
   scale_fill_continuous(
     low = "white", high = "purple", name = "total per 1000", label = scales::comma
   ) + 
...
plot_usmap(
   data = liq_sum_by_state, values = "total_liq", lines = "blue"
 ) + 
   scale_fill_continuous(
     low = "white", high = "purple", name = "total per 1000", label = scales::comma
   ) + 
...
plot_usmap(
   data = relig_sum_by_state, values = "total_relig", lines = "blue"
 ) + 
   scale_fill_continuous(
     low = "white", high = "purple", name = "total per 1000", label = scales::comma
   ) + 
...
```
#state w/ #does not account for the different number of people in each state
...
plot_usmap(
   data = education_state, values = "proportion", lines = "blue"
 ) + 
   scale_fill_continuous(
...
#proportion of college degree holders is different by state
...
plot_usmap(
   ...
   scale_fill_continuous(
     low = "white", high = "pink", name = "proportion", label = scales::comma
   ) + 
...
training_samples <- new_com_data$college_edu |>
  createDataPartition(p = 0.8, list = FALSE)

train_data  <- new_com_data[training_samples, ]
test_data <- new_com_data[-training_samples, ]
```
...
#comeback to interpret and understand what the numbers mean
```
...
# Fit the model
logistic <- glm (college_edu ~ performing_arts + libraries + amusement_parks 
                 + recreation 
                 + fitness 
                 + liquor_store 
                 + tobacco
...
# 
# predicted_classes <- ifelse(probs > 0.5, "0", "1")
# 
# # Model accuracy
# mean(predicted_classes == test_data$college_edu)

#I dont know if this is accurate. Need to return to and modify

```
#try to balance classes
```{r}
balance_class_data <- ubUnderSample(new_com_data, college_edu)
