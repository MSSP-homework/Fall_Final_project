...
#There is a max of 1 in each of the education columns. Thats odd that there is a census tract where all residents have the same type of education. May need to dig deeper and find out what neighborhood these are. It makes sense that the social infrastructures min is 0 but gosh theres a city with 2000 performing arts centers. Crazy.

#The median and mean look pretty standard for all variables

#maybe I could make a table for this
```
...
ggplot(community_data, aes(x = less_than_hs)) +
  ...
  labs(title = "Less than High School Education", x = "Values", y = "Frequency")

#skewed data and outliers 0 and 1. What this mean
```
```{r}
min_school <- community_data |>
  filter(less_than_hs == 1.00)
```

```{r}
ggplot(community_data, aes(x = hs_some_college)) + 
  geom_histogram(bins = 30, fill = "orange", color = "black")+
  labs(title = "High School and Some College Education", x = "Values", y = "Frequency")
```

```{r}
highschool_outlier <- community_data |>
  filter(hs_some_college == 1.00)
```

```{r}
ggplot(community_data, aes(bach_and_higher))+
  geom_histogram(bins = 30, fill = "lightblue", color = "black")+
  labs(title = "Bachelor's degree or Higher", x = "Values", y = "frequency")
```

```{r}
college_outlier <- community_data |>
  filter(bach_and_higher == 1.00)
```

#my data is skewed and I am not sure what to do with thi information or how it will inform my choice of model. There also seems to be some outliers. How do I investigate them and what should I do with them 

#I don't think I need to do any other bar charts or histograms

#check if predictors correlate
```{r}
subset_data <- community_data[, 5:15]
...
#fitness and recreation are highly correlated. I should decide which to drop. All others are fine
```
```{r}
#figure out how to see the correlation between the predictor and the targets
