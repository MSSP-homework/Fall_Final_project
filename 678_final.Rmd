...
#taking samples from each census region
```{r}
# set.seed(123)  # Setting seed for reproducibility
# 
# sampled_art <- art_17_filt |>
#   group_by(census_region) |>
#   sample_n(size = 1500, replace = FALSE) |>
#   ungroup()

#probably pull the samples after everything is matched
```
...
community_data <- community_data |>
  select(-census_region.y, -year.y, -population.y, -census_region.x.x, -census_region.y.y)
```
...
```{r}
 ...
 final_community_data <- community_data |>
   group_by(census_region) |>
   sample_n(size = 1500, replace = FALSE) |>
   ungroup()
```
