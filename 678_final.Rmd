...
community_data <- community_data |>
  pivot_longer(cols = c(less_than_hs, hs_some_college, bach_and_higher),
...
```{r}
 test <- community_data |>
   ...
   summarise(mean_value = mean(pop_proportion), median_value = median(pop_proportion))

# sum_by_category <- community_data %>%
#   summarize(sum_a = sum(co), sum_b = sum(b), sum_c = sum(c))

ggplot(community_data, aes(x = pop_proportion)) +
  geom_histogram(bins = 30, fill = "pink", color = "black") +
  labs(title = "Education", x = "Population Proportion", y = "Number of Neighborhoods")+
  ylim(0, 15000)
```
...
ggplot(community_data, aes(x = education_level)) +
  geom_histogram(bins = 30, fill = "pink", color = "black") +
...
```{r}
ggplot(community_data, aes(bach_and_higher))+
...
#fitness and recreation are highly correlated. I should decide which to drop. All others are fine. I could see which one has higher correlation with my predictor variables and then decide by that 
```
