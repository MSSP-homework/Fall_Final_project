...
```{r}
community_data$no_CD <- community_data$less_than_hs + community_data$hs_some_college #add columns together

community_data <- community_data|> #get rid of dupes and rename
  select(-less_than_hs, -hs_some_college)|>
  rename(CD = bach_and_higher )

community_data$college_edu <- ifelse(community_data$CD > 0.5, 1, 0)
...
#correlation for college degree
```{r}
subset_data <- community_data[, 5:17]
...
ggplot(data = reshape2::melt(cor_matrix), aes(Var2, Var1, fill = value)) +
  ...
  labs(title = "No College Correlation Heatmap", x = "Variables", y = "Variables") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  
```
#no college degree
```{r}
subset_data_2 <- community_data[, 5:18]|>
  select(-13)
  
cor_matrix <- cor(subset_data_2)
