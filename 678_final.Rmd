...
```{r}
complete_pooling <- glm(college_edu ~ libraries 
                 + amusement_parks
                 + liquor_store 
                 + tobacco, 
                 data = train_data, 
...
predicted_probs_2 <- predict(null, newdata = test_data, type = "response")
...
#no pooling
```{r}
no_pooling <- glmer(college_edu ~ libraries + amusement_parks + liquor_store + tobacco + (1 | state), 
                          data = train_data, 
                          family = binomial(link = "logit"))

summary(no_pooling, corr = FALSE)

predicted_probs_3 <- predict(null, newdata = test_data, type = "response")
...
partial_pooling <- glmer(college_edu ~ libraries + amusement_parks + liquor_store + tobacco + (1 + libraries + amusement_parks + liquor_store + tobacco | state), 
                               data = train_data, 
                               family = binomial(link = "logit"),
                         nAGQ = 20)
...
predicted_probs_4 <- predict(null, newdata = test_data, type = "response")
...
#varying slope model
```{r}
<!-- slope_vary <- glmer(college_edu ~ libraries + amusement_parks + liquor_store + tobacco +  -->
<!--                       (libraries + amusement_parks + liquor_store + tobacco | state),  -->
<!--                     data = train_data, -->
<!--                     family = binomial(link = "logit")) -->

<!-- summary(slope_vary) -->

<!-- predicted_probs_5 <- predict(null, newdata = test_data, type = "response") -->
<!-- predicted_classes_5 <- ifelse(predicted_probs_5 > 0.5, 1, 0) -->
<!-- actual_classes_5 <- test_data$college_edu -->

<!-- # Calculate misclassification error -->
<!-- misclassification_error_5 <- mean(predicted_classes_4 != actual_classes_5) -->
