...
art_17 <- art|>
  filter(year == "2017")|>
  select(tract_fips10, year, population, count_7111, count_7112, count_712, count_51912, 
        count_7131, count_7132, count_7139, count_71394)
```
...
vice_17 <- vice|>
  filter(year == "2017")|>
  select(tract_fips10, year, population, count_4453, count_453991)
```
...
social_orgs_17 <- social_orgs|>
  filter(year == "2017")|>
  select(tract_fips10, year, population, count_8131, count_8134)
```
...
community_data <- community_data |>
 ...
 community_data <- community_data |>
   rename(
     performing_arts = count_7111,   
     spectator_sports_orgs = count_7112,
     museums = count_712,  
     libraries = count_51912,  
     amusement_parks = count_7131,  
     casinos = count_7132,
     recreation = count_7139,  
     fitness = count_71394,  
     liquor_store = count_4453,  
     tobacco = count_453991,
     religious_orgs = count_8131,   
     social_orgs = count_8134,  
...
### EDA Begins ###
#understand data and how I want to model it
...
#time to check the variable distributions
```{r}

ggplot(community_data, aes(x = less_than_hs)) +
  geom_histogram(bins = 30, fill = "pink", color = "black") +
  labs(title = "Less than High School Education", x = "Population Proportion", y = "Number of Neighborhoods")+
  ylim(0, 15000)
```
```{r}
#It is skewed so that means that I need to transform it so that it will be normalized and better for analysis

community_data$cubed_less_than_hs <- community_data$less_than_hs^(1/3)

ggplot(community_data, aes(x = cubed_less_than_hs)) +
  geom_histogram(bins = 30, fill = "pink", color = "black") +
  labs(title = "Less than High School Education", x = "Population Proportion", y = "Number of Neighborhoods")+
  ylim(0, 15000)

#should i check for normality now?

qqnorm(community_data$cubed_less_than_hs) #what the hell does this mean
```

```{r}
ggplot(community_data, aes(x = hs_some_college)) + 
  geom_histogram(bins = 30, fill = "orange", color = "black")+
  labs(title = "High School and Some College Education", x = "Population Proportion", y = "Number of Neighborhoods")
#1 neighborhood where all of the residents have a high school degree
```
```{r}
community_data$sqrt_hs_some_college <- sqrt(community_data$hs_some_college)

ggplot(community_data, aes(x = sqrt_hs_some_college)) +
  geom_histogram(bins = 30, fill = "orange", color = "black") +
  labs(title = "Sqrt High School and Some College Education", x = "Population Proportion", y = "Number of Neighborhoods")
```

```{r}
community_data$cubed_bach_and_higher <- community_data$bach_and_higher^(1/3)

ggplot(community_data, aes(cubed_bach_and_higher))+
  geom_histogram(bins = 30, fill = "lightblue", color = "black")+
  labs(title = "Bachelor's degree or Higher", x = "Proportion of Population", y = "Number of Neighborhoods")+
  ylim(0,7500)
```

#check if predictors correlate
```{r}
subset_data <- community_data[, 5:19]
...
#fitness and recreation are highly correlated. I should decide which to drop. All others are fine. I could see which one has higher correlation with my predictor varibles and then decide by that 
```
```{r}
#code to get ride of fitness. It is highly correlated with recreation so we will get rid of it because neither one of them seem to be making much of difference,

#I dont think I need to remove them from the dataset I just don't need to include in my model

#my response variables are correlated. Doesn't this mean I should do MLM
```


#map of US w/ dots showing the social infrastructure 
```{r}

```

#map of US with education
```{r}
idk <- data.frame


please <- plot_usmap(community_data, less_than_hs)


# sf_us_states <- st_as_sf(maps::map("state", plot = FALSE, fill = TRUE))
# eda_multilevel$State <- tolower(eda_multilevel$STATE_NAME)
#  
# sf_merged <- merge(sf_us_states, eda_multilevel, by.x = "ID", by.y = "State")
#  
#  
# # Heatmap     
# ggplot() +
#   geom_sf(data = sf_merged, aes(fill = topsis_score)) +
#   scale_fill_gradient(low = "pink", high = "red") +
#   labs(
#     title = "Heatmap of Evaluating Results",
#     fill = "evaluating results"
#   ) +
#   theme_minimal()
```
