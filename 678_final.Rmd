...
```
#look for perfect for seperation and get rid of rows with all 0's
```{r}
# subset <- new_com_data[ , 6:16]|>
#   dplyr::select(-9, -10)
# 
# all_zeros_rows <- which(rowSums(subset == 0) == ncol(subset))
# 
# com_data_filtered <- new_com_data[-all_zeros_rows, ]

#don't need to do this but keep around for safety
```
...
```{r}
 balanced_train_data <- ovun.sample(college_edu ~  performing_arts + 
                             libraries + 
                             amusement_parks + 
                             recreation + 
                             fitness + 
                       ...
                       N = 57928, 
                       seed = 678)$data
 table(balanced_train_data$college_edu)
```
...
predicted_probs <- predict(null, newdata = test_data, type = "response")

predicted_classes <- ifelse(predicted_probs > 0.5, 1, 0)
actual_classes <- test_data$college_edu
...
```{r}
logistic <- glm(college_edu ~ libraries 
                 + amusement_parks
                 + liquor_store 
                 + tobacco, 
                 data = train_data, 
...
logistic <- glm(college_edu ~ + libraries 
 ...
 no_pooling <- lm(college_edu ~ group)
```
...
partial_pooling <- glmer(college_edu ~ libraries 
                 + amusement_parks
                 + liquor_store 
                 + tobacco 
                 +(1 | state),
